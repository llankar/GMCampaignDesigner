...
from modules.audio.ui.bar.formatters import (
    format_track_label,
    section_button_label,
    track_identifier,
    truncate_with_suffix,
)
from modules.audio.ui.bar.interactions import resolve_audio_filters
from modules.audio.ui.bar.layout import (
    NOW_PLAYING_LABEL_MAX_CHARS,
    NOW_PLAYING_MENU_WIDTH,
    SEARCH_RESULTS_LABEL_MAX_CHARS,
    SEARCH_RESULTS_MENU_WIDTH,
)
...
class AudioBarWindow(ctk.CTkToplevel):
    ...
    def __init__(
        ...
        *,
        ...
        self.section_toggle_var = tk.StringVar(value=section_button_label(self._active_section))
        ...
        self._search_results_label_max_chars = SEARCH_RESULTS_LABEL_MAX_CHARS
        self._now_playing_label_max_chars = NOW_PLAYING_LABEL_MAX_CHARS
        self._build_ui()
    ...
    def _toggle_section(self) -> None:
        ...
        self.section_toggle_var.set(section_button_label(self._active_section))
    ...
    def _on_category_selected(self, choice: str) -> None:
        ...
        resolved_category, resolved_mood, moods = resolve_audio_filters(
            library=library,
    ...
    def _perform_search(self) -> None:
        ...
        lookup, values, placeholder = build_search_lookup(results, self._search_results_label_max_chars)
    ...
    def _apply_search_result(self, info: Dict[str, Any]) -> None:
        ...
        if current_category == category:
            ...
            if label:
                ...
                self._remembered_track_label = truncate_with_suffix(
                    format_track_label(track) or self._remembered_track_label or "",
    ...
    def _refresh_from_state(self, section: Optional[str] = None) -> None:
        ...
        if track:
            ...
            label = format_track_label(track) or ""
            self._remembered_track_label = (
                truncate_with_suffix(label, self._now_playing_label_max_chars) if label else None
            )
    ...
    def _apply_selected_filters(
        ...
        *,
        ...
        resolved_category, resolved_mood, _ = resolve_audio_filters(
            library=library,
            section=self._active_section,
    ...
    def _update_playlist_menu(self, playlist: list[Dict[str, Any]]) -> None:
        ...
        self._playlist_lookup, values = build_playlist_lookup(
            playlist,
            self._now_playing_label_max_chars,
        )
