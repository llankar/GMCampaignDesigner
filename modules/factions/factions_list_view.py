...
from .factions_model import load_factions, save_factions, load_template
from .faction_editor_view import EditFactionWindow
import uuid
...
class FactionWindow(ctk.CTkToplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Faction Management")
        ...
        self.search_entry = ctk.CTkEntry(search_frame, placeholder_text="Search Faction")
        self.search_entry.pack(side="left", expand=True, fill="x", padx=5)
        self.search_entry.bind("<KeyRelease>", lambda e: self.filter_items())

        add_button = ctk.CTkButton(search_frame, text="Add Faction", command=self.add_item)
    ...
    def load_item_list(self):
        ...
        if not self.items:
            ctk.CTkLabel(self.table_frame, text=f"No Factions found.").grid(row=1, column=0, columnspan=len(self.fields), pady=5)
        else:
    ...
    def add_item(self):
        ...
        if editor.saved:
            ...
            save_factions(self.items)
            self.load_item_list()

    def edit_item(self, item):
        editor = EditFactionWindow(self, item)
        self.wait_window(editor)
        if editor.saved:
            save_factions(self.items)
            self.load_item_list()
    ...
    def filter_items(self):
        ...
        self.items = [i for i in load_factions() if any(query in str(v).lower() for v in i.values())]
        self.load_item_list()