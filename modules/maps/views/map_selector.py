...
def _on_display_map(self, entity_type, map_name): # entity_type here is the map's default, not token's
    ...
    previous_map = getattr(self, "current_map", None)
    if previous_map and getattr(self, "tokens", None):
        try:
            self._persist_tokens()
        except Exception as exc:
            print(f"[_on_display_map] Failed to persist outgoing map '{previous_map.get('Name')}' before switch: {exc}")

    suspend_depth = getattr(self, "_suspend_token_persistence", 0)
    self._suspend_token_persistence = suspend_depth + 1
    ...
    try:
        ...
        self._suspend_token_persistence = suspend_depth
