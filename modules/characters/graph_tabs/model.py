"""Utilities for graph tabs model."""
...
def _new_tab_id() -> str:
    """Internal helper for new tab ID."""
...
def ensure_graph_tabs(graph: Dict) -> None:
    ...
    if not isinstance(tabs, list) or not tabs:
        # Handle the branch where not isinstance(tabs, list) or tabs is unavailable.
...
def filter_graph_for_tab(graph: Dict, tab: Dict) -> Tuple[List[Dict], List[Dict]]:
    ...
    for node in nodes:
        ...
        if entity_types and node.get("entity_type") not in entity_types:
            continue
        if search:
            # Continue with this path when search is set.
            haystacks = [
