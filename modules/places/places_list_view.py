...
from .places_model import load_places, save_places, load_template
from .place_editor_view import EditPlaceWindow
import uuid
...
class PlaceWindow(ctk.CTkToplevel):
    def __init__(self, master):
        ...
        self.search_entry = ctk.CTkEntry(search_frame, placeholder_text="Search Place")
        self.search_entry.pack(side="left", expand=True, fill="x", padx=5)
        self.search_entry.bind("<KeyRelease>", lambda e: self.filter_items())

        add_button = ctk.CTkButton(search_frame, text="Add Place", command=self.add_item)
    ...
    def load_item_list(self):
        ...
        if not self.items:
            ctk.CTkLabel(self.table_frame, text=f"No Places found.").grid(row=1, column=0, columnspan=len(self.fields), pady=5)
        else:
    ...
    def add_item(self):
        ...
        editor = EditPlaceWindow(self, new_item, creation_mode=True)
        self.wait_window(editor)
        if editor.saved:
            self.items.append(new_item)
            save_places(self.items)
            self.load_item_list()

    def edit_item(self, item):
        editor = EditPlaceWindow(self, item)
        self.wait_window(editor)
        if editor.saved:
            save_places(self.items)
            self.load_item_list()
    ...
    def filter_items(self):
        ...
        self.items = [i for i in load_places() if any(query in str(v).lower() for v in i.values())]
        self.load_item_list()