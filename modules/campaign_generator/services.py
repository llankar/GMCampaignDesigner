...
def _empty_legacy_formatting() -> dict[str, list]:
    """Return a fresh legacy formatting payload with independent lists."""
    return {
        "bold": [],
        "italic": [],
        "underline": [],
...
def build_scenario_export_payload(campaign: Dict[str, str]) -> dict:
    """Build the JSON payload used by the legacy export action."""
    quest_text = campaign.get("Quest", "Adventure")
    title_candidate = _split_legacy_label(quest_text)
    ...
    start_place = _split_legacy_label(start_loc)
    if start_place and start_place != start_loc.strip():
        places.append(start_place)
    region = campaign.get("Region", "")
    region_parts = []
    region_label = _split_legacy_label(region)
    if region_label and region_label != region.strip():
        region_parts.append(region_label)
    ...
    return {
        ...
        "Summary": {"text": summary_text, "formatting": _empty_legacy_formatting()},
        "Secrets": {"text": secrets_text, "formatting": _empty_legacy_formatting()},
        "Places": places,
        "NPCs": npcs,
    }
...
def append_scenario_to_json_file(campaign: Dict[str, str], filename: str) -> None:
    ...
    try:
        ...
        if isinstance(existing, dict):
            scenarios = existing.get("scenarios", []) if isinstance(existing.get("scenarios", []), list) else []
    ...
    with open(filename, "w", encoding="utf-8") as handle:
        json.dump({"scenarios": scenarios}, handle, ensure_ascii=False, indent=4)
